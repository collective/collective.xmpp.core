...
for lang in $(find locales -mindepth 1 -maxdepth 1 -type d); do
    ...
    if test -d $lang/LC_MESSAGES; then
        ...
        PO=$lang/LC_MESSAGES/${JSCATALOGNAME}.po
        ...
        # Sync po file
        echo "Syncing $PO"
        i18ndude sync --pot locales/$JSCATALOGNAME.pot $PO

        # Compile .po to .mo
        MO=$lang/LC_MESSAGES/${JSCATALOGNAME}.mo
        echo "Compiling $MO"
        msgfmt -o $MO $lang/LC_MESSAGES/${JSCATALOGNAME}.po
    fi
done
